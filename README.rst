...
the name ``default`` and a default value of 12. So does the parser !!!
...
``parse_this`` is looking for an ``int``, a ``str`` and potentially and
other ``int`` and that's just what we have provided. When effectively
parsing the command line, via ``parser.parse_args()``, we are given an
object that contain the expected attributes ``an_int``, ``a_string`` and
``default`` and their value will come directly from the command line
arguments and options.
...
The output will be the same as above. The only difference is the use of
the special value ``Class`` to specify where the ``cls`` argument is
used.
